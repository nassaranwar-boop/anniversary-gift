...
(async () => {
  ...
  await p.goto('http://127.0.0.1:8899/tools/nightshot.html', { waitUntil: 'domcontentloaded', timeout: 60000 });
  await p.waitForTimeout(400);
  const build = await p.evaluate(() => {
    const t0 = performance.now();
    OuissysNightShift.preview(document.getElementById('stage'), document.getElementById('cvs'), 'office', 'main');
  ...
  for (const room of ['office', 'hall', 'stage', 'party', 'arcade', 'foyer', 'workshop', 'closet', 'ducts']) {
    const m = await p.evaluate((r) => {
      ...
      OuissysNightShift.preview(stage, cvs, r, 'main');
      const t0 = performance.now();
      for (let i = 0; i < 20; i++) OuissysNightShift.preview(stage, cvs, r, 'main');
      const ms = (performance.now() - t0) / 20;
      const info = OuissysNightShift.__three ? OuissysNightShift.__three().renderer.info.render : null;
