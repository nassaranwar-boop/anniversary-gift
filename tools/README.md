...
| `landscape.js` | **run this when he mentions holding the phone sideways.** Asserts the two things a short wide screen breaks: that every control is actually on the screen (or inside something that scrolls far enough to bring it there), and that the game stages use the room they are given. It catches both of the faults it was written for — put the old CSS back and it reports 5 failures, including `#btn-start @y375..424` in a 390px window, which is the button that opens the maze. `regress.js` would never have found that: it checks for horizontal scroll and page errors, not whether a button is reachable. Note the ancestor-visibility walk in it — the apocalypse key pad is `opacity:0; height:0` on touch devices and counting it gave a false alarm on the first run. |
