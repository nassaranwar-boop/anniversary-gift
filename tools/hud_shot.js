...
(async () => {
  ...
  const fill = process.env.FILL ? +process.env.FILL : 13;
  await page.evaluate(f => { window.__soSetMeter(f); window.__soTele(12); window.__soPump(0.4, {}); }, fill);
  await page.waitForTimeout(200);
  await page.evaluate(() => { window.__soHalt(); const st=document.createElement('style');
    st.textContent='*{backdrop-filter:none !important}'; document.head.appendChild(st); });
  const cdp = await page.context().newCDPSession(page);
  const { data } = await cdp.send('Page.captureScreenshot', { format: 'png' });
  fs.writeFileSync(process.argv[2] || 'hud.png', Buffer.from(data, 'base64'));
